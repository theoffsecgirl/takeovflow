    ...
    with ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        for future in as_completed(futures):
            ...
            if result:
                ...
                log("  {} [cname] {}".format(SEVERITY_COLOR.get(result["severity"], "⚪"), line))
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="takeovflow_tmp_") as _tmpdir:
        ...
        print("\n" + "=" * 60, file=sys.stderr)
        print("[OK] Análisis completado.", file=sys.stderr)
        print("     Markdown  : {}".format(report_md), file=sys.stderr)
        if report_json:
            print("     JSON      : {}".format(report_json), file=sys.stderr)
        if findings_out:
            print("     Findings  : {}".format(findings_out), file=sys.stderr)
        total_findings = len(normalized_all)
        if total_findings:
            print("     ⚠️  {} posible(s) takeover(s) encontrado(s)".format(total_findings), file=sys.stderr)
        else:
            print("     ✅ Sin takeovers detectados", file=sys.stderr)
